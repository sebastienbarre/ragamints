 ...
 *
 * @param {String} stringified_entry stringified string to compress
 * @return {String} compressed cache entry
 */
api.compress = function(stringified_entry) {
 ...
 */
api.decompress = function(compressed_entry) {
  return encoding === 'utf8'
 ...
 */
api.set = function(key, value, ttl) {
  if (!enabled) {
  ...
  let compressed_entry = api.compress(stringified_entry);
  let use_compression =
...
api.get = function(key) {
  ...
  if (typeof entry === 'string') {
    let decompressed_entry = api.decompress(entry);
