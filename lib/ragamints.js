...
function logForMedia(media, msg) {
  ...
  log.output(log.notice(`${index} [${excerpt}]`), (msg ? msg : ''));
}
...
function resolveMediaId(media_id) {
  ...
  return fetch(endpoint).then(function(response) {
    ...
    log.output('Found media ID:', log.success(json.media_id),
...
function resolveOptions(options) {
  ...
  if (resolved_options.accessToken === undefined) {
    ...
    if (process.env && process.env.hasOwnProperty(env_var)) {
      ...
      log.output('Using', log.success(env_var),
  ...
  for (let timestamp in timestamps) {
    if (options[timestamp]) {
      ...
      log.output(timestamps[timestamp] + ':', log.notice(options[timestamp]),
  ...
  return new Promise(function(resolve, reject) {
    suspend(function*() {
      try {
        ...
        let all_medias = [];
        let it = user.getRecentMedias(options.userId, options);
        ...
        log.output('Done processing', log.success(all_medias.length),
          'media(s). Easy peasy.');
        resolve(all_medias);
      }
      catch (err) {
...
function main(argv) {
  ...
  program.option(
    ...
    log.output('  Check the man page or README file for more.');
  }).parse(argv);
