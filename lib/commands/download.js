...
function getMediaBasenameForResolution(media_obj, resolution) {
  ...
  let suffix = media_obj.type === 'image' && resolution !== undefined ? '-' + resolution : '';
...
function saveMediaObject(media_obj, options) {
  return new Promise(function(resolve, reject) {
    ...
    let keys = options.json && options.json.constructor === Array ? options.json : null;
...
function resolveOptions(options) {
  ...
  for (let timestamp in timestamps) {
    if (options[timestamp]) {
      resolved_options[timestamp] = core.utils.isUnixTimestamp(options[timestamp])
        ? options[timestamp]
...
function run(unresolved_options) {
  ...
  function processMedia(media_obj, options) {
    ...
    if (options.resolution !== false) {
      ...
      resolutions = _values(_indexBy(resolutions, getUrlToResolution.bind(this, media_obj)));
      resolutions.forEach(function(resolution) {
        promises.push(
          fetchMedia(media_obj, resolution, options).then(function(filename) {
            return q(updateFileMetadata.bind(this, media_obj, filename, options));
          })
        );
      });
    }
