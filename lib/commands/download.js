...
let cliOptions = {
  ...
  'r': {
    ...
    describe: 'Resolution(s) to download, e.g. ' +
      _values(instagram.client.RESOLUTIONS).join(','),
    type: 'string',
  }
};
...
/**
 * Get the exiftool arguments needed to update the metadata for a given media,
 * pertaining to its user metadata.
 *
 * @param {Object} media_obj Media object
 * @return {Array} array of command-line arguments
 */
...
function getExifToolCreatedArgs(media_obj, options) {
  ...
  if (media_obj.location) {
    ...
    if (options && options.verbose && !options.quiet) {
      ...
      instagram.media.log(
        media_obj, 'Timezone is ' + core.logger.success(tz));
      instagram.media.log(
        media_obj,
        'Creation time stored as ' + core.logger.success(formatted));
    }
  } else {
    if (options && options.verbose && !options.quiet) {
      ...
      instagram.media.log(
        media_obj,
        'No location, assume timezone ' + core.logger.warn('local'));
      instagram.media.log(
        media_obj,
        'Creation time stored as ' + core.logger.success(formatted));
    }
  ...
  return new Promise(function(resolve, reject) {
    ...
    exiftool.on('close', function() {
      if (error_message) {
        ...
        if (!options.quiet) {
          instagram.media.log(
            media_obj, 'Updated metadata in ' + core.logger.success(basename));
        }
        resolve(true);
      }
    });
...
function getUrlToResolution(media_obj, resolution) {
  if (resolution === instagram.constants.RESOLUTIONS.high ||
...
function getMediaBasenameForResolution(media_obj, resolution) {
  let name = instagram.media.createMediaFileName(media_obj);
    ...
    fs.lstat(filename, function(lstat_err, stats) {
      ...
      if (!lstat_err &&
        ...
        if (!options || !options.quiet) {
          instagram.media.log(
            media_obj, 'Saved already as ' + core.logger.success(basename));
        }
        resolve(filename);
      } else {
        new Download()
        ...
        .run(function(download_err, files) {
          if (download_err) {
            ...
            if (!options || !options.quiet) {
              ...
              instagram.media.log(
                media_obj, 'Fetched ' + core.logger.success(file_basename));
            }
      ...
      if (mkdirp_err) {
        ...
        fs.writeFile(filename, stringified, function(err) {
          if (err) {
            ...
            if (!options.quiet) {
              instagram.media.log(
                media_obj, 'Saved ' + core.logger.success(basename));
            }
            resolve(filename);
          }
        });
 ...
 * resolve timestamps, resolve media URL to media id, etc.
 *
...
function resolveOptions(options) {
  ...
  for (let timestamp in timestamps) {
    if (options[timestamp]) {
      resolved_options[timestamp] =
        core.utils.isUnixTimestamp(options[timestamp])
        ? options[timestamp]
        : Math.floor(Date.create(options[timestamp]).getTime() / 1000);
      core.logger.log(
        timestamps[timestamp] + ':', core.logger.notice(options[timestamp]),
        'is',
        core.logger.success(moment.unix(resolved_options[timestamp]).format()),
  ...
  let resolvers = Promise.all([
    ...
    instagram.user.resolveUserId(options.instagramUserId).then(function(id) {
      resolved_options.instagramUserId = id;
    }),
    instagram.media.resolveMediaId(options.minId).then(function(id) {
      resolved_options.minId = id;
    }),
    instagram.media.resolveMediaId(options.maxId).then(function(id) {
      resolved_options.maxId = id;
    })
  ]);
  return resolvers.then(function() {
...
function run(unresolved_options) {
  ...
  return resolveOptions(unresolved_options).then(function(options) {
    return instagram.user.forEachRecentMedias(
      options.instagramUserId, options, processMedia);
  });
}
