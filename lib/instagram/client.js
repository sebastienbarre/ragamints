...
const ig_node = require('instagram-node').instagram();
...
function user_search_cache_key(user_name, options) {
  const hash = core.cache.hash(options);
 ...
 */
api.user_search = (user_name, options, callback) => {
  const cache_key = user_search_cache_key(user_name, options);
  core.cache.get(cache_key).then((users) => {
    // core.logger.log(
    ...
    ig_node.user_search(
      user_name,
      options,
      (err, users, remaining, limit) => { // eslint-disable-line max-params
        if (!err && users.length) {
 ...
 * @param {Function} callback - The callback.
 */
api.user_media_recent = (user_id, options, callback) => {
  ...
  const optimized_options = assign({}, options);
  ...
  const cache_key = user_media_recent_cache_key(user_id, optimized_options);
  const all_medias = [];
  ...
  function handler( // eslint-disable-line max-params, require-jsdoc
    ...
    if (pagination && pagination.next) {
      const old_next = pagination.next;
      pagination.next = (cb) => { // eslint-disable-line no-param-reassign
        next_callback = cb;
        old_next(handler);
      };
    }
  ...
  core.cache.get(cache_key).then((medias) => {
    // core.logger.log(
    // 'user_media_recent for', core.logger.notice(user_id), 'in cache');
    const sliced = options.count && medias.length > options.count;
 ...
 */
api.oembed = (media_url) => {
  const cache_key = oembed_cache_key(media_url);
  return core.cache.get(cache_key).then(
    oembed => oembed,
    // core.logger.log(
    // 'oembed for', core.logger.notice(media_url), 'in cache');
    () => {
      const url = `http://api.instagram.com/oembed?callback=&url=${media_url}`;
