...
const cliOptions = {
  command: {
    't': {
      alias: constants.ACCESS_TOKEN_CLI_OPTION,
      describe: 'Instagram Access Token',
      type: 'string'
    },
    'u': {
      alias: constants.USER_ID_CLI_OPTION,
  ...
  if (resolved_options.instagramAccessToken === undefined) {
    ...
    if (process.env && process.env.hasOwnProperty(env_token)) {
      ...
      return Promise.reject(
        new core.logger.Error('Need Instagram access token'));
    }
  }
  client.use({
 ...
 *
 * @param  {String} user_id_or_name Instagram User ID or User Name
 * @return {Promise} resolving with Instagram user ID, or rejecting
 */
    ...
    client.user_search(
      user_id_or_name, {
        ...
        if (err) {
          ...
          reject(new core.logger.Error(msg));
        } else {
  ...
  return new Promise(function(resolve, reject) {
    suspend(function*() {
      try {
        ...
        // In sequential mode, let's use the separate variable we had set up to
        ...
        if (skipped_count) {
          core.logger.log(
            'Skipped', core.logger.success(skipped_count), 'videos(s).');
        }
        core.logger.log(
          'Done iterating over',
          core.logger.success(output.length),
          'media(s). Easy peasy.');
        resolve(output);
      } catch (err) {
