    ...
    ig.user_search(
      user_id, {
        ...
        if (err) {
          ...
          reject(new logger.Error(`Could not find user ID for: ${user_id}`));
        } else {
          logger.log('Found user ID:', logger.success(users[0].id),
            'for username:', logger.notice(user_id));
          resolve(users[0].id);
        }
      });
  });
...
function getRecentMedias(user_id, options) {
  let current_count = 0;
  let requested_count = options.count || 10;
  ...
  let handler = function handler(resolve, reject, err, medias, pagination) {
    ...
    if (pagination.next &&
        (!requested_count ||
         (requested_count && current_count < requested_count))) {
      next = new Promise(function(next_resolve, next_reject) {
        pagination.next(handler.bind(null, next_resolve, next_reject));
      });
    } else if (requested_count && current_count > requested_count) {
      let pos = requested_count - current_count;
...
module.exports = {
  ...
  getRecentMediasCliOptions: getRecentMediasCliOptions
};
